function execJSONPath(str)
{
 var json = JSON.parse(str);
 var path = getInputString('jsonpath-path');
 var resultType = document.getElementById('jsonpath-rp').checked ? 'path' : document.getElementById('jsonpath-rptr').checked ? 'pointer' : 'value';
 
 return JSON.stringify(JSONPath({resultType: resultType}, path, json));
}
...
//from https://github.com/s3u/JSONPath

(function (glbl) {'use strict';
...
JSONPath.prototype._eval = function (code, _v, _vname, path, parent, parentPropName) {
    ...
    if (code.includes('@parentProperty')) {
        var _$_parentProperty = parentPropName;
        code = code.replace(/@parentProperty/g, '_$_parentProperty');
    }
    if (code.includes('@parent')) {
        var _$_parent = parent;
        code = code.replace(/@parent/g, '_$_parent');
    }
    if (code.includes('@property')) {
        var _$_property = _vname;
        code = code.replace(/@property/g, '_$_property');
    }
    if (code.includes('@path')) {
        var _$_path = JSONPath.toPathString(path.concat([_vname]));
